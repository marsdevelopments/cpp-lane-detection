...
constexpr float trapezoidBottomWidth = 0.6f; // Width of bottom edge of trapezoid, expressed as percentage of image width
constexpr float trapezoidTopWidth = 0.05f;   // Above comment also applies here, but then for the top edge of trapezoid
constexpr float trapezoidHeight = 0.23f;     // Height of the trapezoid expressed as percentage of image height

// Parameters small trapezoid
constexpr float smallBottomWidth = 0.45f; // This will be added to trapezoidBottomWidth to create a less wide bottom edge
constexpr float smallTopWidth = 0.3f;     // We multiply the percentage trapoezoidTopWidth with this parameter to create a less wide top edge
constexpr float smallHeight = 1.0f;       // Height of the small trapezoid expressed as percentage of height of big trapezoid
...
{
    Mat dst;
    GaussianBlur(source, dst, Size(5, 5), 0);
...
Mat drawLanes(Mat source, std::vector<Vec4i> lines)
{
    ...
    int y2 = source.rows * (1 - trapezoidHeight); // Y coordinate of ending point of both the left and right lane
...
std::vector<Vec4i> houghLines(Mat canny, Mat source, bool drawHough)
{
    ...
    double theta = 1 * 3.14 / 180; // Angular resolution in radians of the Hough grid
